...
- 配布用スキル本体: `./skills/context-diet/SKILL.md` と `./skills/context-diet/references/`
  （**このリポの開発ルールではなく配布物**）
- plugin / marketplace manifest: `./.claude-plugin/plugin.json` / `./.claude-plugin/marketplace.json`（同じく配布物）
...
- **`skills/context-diet/` 配下は利用者の環境へ配布されて動く実体。** 作者環境固有の
  絶対パス・個人情報・kb 由来の固有名詞を書かない。`CLAUDE.md`（このリポを開発する
  AI 向け）と混同しない。スキルを変更したら `.claude-plugin/plugin.json` の `version`
  を上げる
- **実測していない削減テクニックを載せない。** 数値の裏付けが無いものを足すと、この
  リポの価値そのものが失われる。測り方は API usage（`claude -p … --output-format json` の
  `usage` 合計）を正とし、`/context` の値は推定として併記する（同条件で 14.6k tokens ずれた）
